// Normalize a delivered webhook envelope to idiomatic TS: snake_case keys → camelCase,
// and a known set of numeric fields that the wire serializes as strings (bigints) →
// numbers. Opaque subtrees are passed through untouched: `raw` (source-specific adapter
// data), `metadata`, `refs`, and `vars` (caller/source-owned maps — must round-trip
// verbatim, keys included). Used by `verifyWebhook` only; the management client speaks
// camelCase on the wire and needs no normalization.
