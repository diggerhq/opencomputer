// Incremental SHA-256 (FIPS 180-4) over Uint8Array chunks. Web Crypto's
// `digest` takes the whole message at once, and this module graph must stay
// portable (no Node `crypto`), so a streamed artifact download hashes with
// this and compares the result to the digest the API sent.
...
export class Sha256 {
  ...
  update(chunk: Uint8Array): this {
    ...
    if (this.blockLength > 0) {
      const take = Math.min(64 - this.blockLength, chunk.byteLength);
    ...
    for (; offset + 64 <= chunk.byteLength; offset += 64) this.compress(chunk, offset);
  ...
  /** The digest as lowercase hex. The hasher cannot be updated afterwards. */
  digestHex(): string {
