...
func (s *Store) Migrate(ctx context.Context) error {
	...
	migrations := []struct {
		...
		{13, "migrations/010_secret_sessions.up.sql"},
	}
...
// UpdateSandboxSessionSecretSession stores the session-based secret credentials for wake-on-request.
func (s *Store) UpdateSandboxSessionSecretSession(ctx context.Context, sandboxID, sessionID, sessionToken string) error {
	_, err := s.pool.Exec(ctx,
		`UPDATE sandbox_sessions SET secret_session_id = $1, secret_session_token = $2 WHERE sandbox_id = $3`,
		sessionID, sessionToken, sandboxID)
	return err
}

// GetSandboxSecretSession returns the secret session credentials for a sandbox, or empty strings if not set.
func (s *Store) GetSandboxSecretSession(ctx context.Context, sandboxID string) (sessionID, sessionToken string, err error) {
	var sid, stok *string
	err = s.pool.QueryRow(ctx,
		`SELECT secret_session_id, secret_session_token FROM sandbox_sessions WHERE sandbox_id = $1 ORDER BY started_at DESC LIMIT 1`,
		sandboxID,
	).Scan(&sid, &stok)
	if err != nil {
		return "", "", fmt.Errorf("get sandbox secret session: %w", err)
	}
	if sid != nil {
		sessionID = *sid
	}
