...
// resolveNamedCheckpoint resolves a named image_cache entry to its underlying checkpoint ID.
// The label parameter is used in error messages (e.g. "snapshot", "image").
// Accepts both names and UUIDs — if the param parses as a UUID, it looks up by ID.
