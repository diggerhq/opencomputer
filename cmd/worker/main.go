...
func main() {
	...
	// LoadSecrets, which is slow and can fail without cloud credentials.
	//
	...
	// Load secrets from the configured cloud secret store before config.Load
	// reads env vars. No-op if neither Azure Key Vault nor AWS Secrets Manager
	// is configured.
		...
		if err != nil {
			...
			go func() {
				...
				for notice := range notices {
					log.Printf("opensandbox-worker: PREEMPTION notice from %s - action=%s eta=%s, notifying resumable sandboxes",
						preemptMon.Name(), notice.Action, notice.ETA.Format(time.RFC3339))
					notifyResumableSandboxesBeforeRestart(context.Background(), mgr, store, sandboxDBMgr, 25*time.Second, notice.ETA)
					hb.Stop()
					return
				}
			}()
...
func notifyResumableSandboxesBeforeRestart(ctx context.Context, mgr sandbox.Manager, store *db.Store, sandboxDBs *sandbox.SandboxDBManager, noticeWindow time.Duration, eta time.Time) {
	if mgr == nil {
		return
	}
	wait := noticeWindow
