...
Templates work on this runtime, but **the templates you have today probably do not.** This is
...
<Warning>
A template that carries a **rootfs image** cannot be used on this runtime. The create fails with
*"template … carries a rootfs image, which this runtime cannot restore."*
</Warning>

Templates built on the current runtime capture the **whole disk** — every system-level change,
...
<Steps>
  ...
  <Step title="Rebuild the ones that fail">
    ...
    [checkpoint](/microvm/checkpoints) or a new template. What can be captured is the
...
supported** on this runtime. They depend on a build fleet it does not have.
</Warning>
