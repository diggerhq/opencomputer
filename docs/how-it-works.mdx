...
Your code talks to a **control plane** API, which provisions a Firecracker microVM on a worker node. Commands, file operations, and agent sessions run inside that VM — events and I/O stream back over WebSocket. The control plane transparently proxies data-plane requests to the correct worker.
...
<Steps>
  ...
  <Step title="Operate">
    Run commands, read/write files, start agent sessions. All execution happens inside the VM. The control plane routes requests to the correct worker automatically.
  </Step>
</Steps>
