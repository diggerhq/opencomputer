...
A sandbox's memory is chosen when it is created and stays fixed for its lifetime, with CPU allocated proportionally. See [Sizing](/sandboxes/sizing) for the available tiers.
