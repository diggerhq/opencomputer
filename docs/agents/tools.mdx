...
- A later call replaces the entire value of the session's `result`.
  Reporting does not finish the turn. Every committed value is also kept as
  its own immutable record, readable by session, turn or id through the
  [results routes](/agents/results).
