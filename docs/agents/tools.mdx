...
| `identity`                 | Where this call runs, assigned by OpenComputer; see [below](#invocation-identity) |
| `sessionId`                | Current session ID; the same as `identity.sessionId`                     |
| `messageId`                | Message containing the tool call; the same as `identity.messageId`       |
| `toolCallId`               | ID of this invocation, unique within the session; `identity.toolCallId`  |
| `agentId`                  | The cloud agent executing the call; the same as `identity.agentId`       |
...
`identity` names the exact execution a call belongs to, using the same IDs
the Management API and the [event log](/agents/events#tools) use:

| Field          | Value                                                                    |
| -------------- | ------------------------------------------------------------------------ |
| `projectId`    | The project the agent belongs to                                         |
| `environment`  | `development` or `production`                                            |
| `agentId`      | The cloud agent, as the Management API identifies it                     |
| `deploymentId` | The deployment pinned to the session when it was created                 |
| `sessionId`    | The session                                                              |
| `turnId`       | The turn OpenComputer admitted that this call belongs to                 |
| `messageId`    | The assistant message that emitted the call                              |
| `toolCallId`   | This call; the `callId` of its `tool.*` events                           |

OpenComputer fills every field from its own record of the session and turn
when it dispatches the call. Nothing in it comes from the prompt, the
session's input or the model's arguments, and the object is frozen for the
whole call: a write to it throws. A model that puts a `turnId`, a
`deploymentId` or a whole `identity` into the tool's arguments only adds
data to `input`; it cannot change or shadow `identity`. Treat such values as
application data at most, and bind your own records to `identity`.

A session keeps its `deploymentId` and `environment` for its lifetime, so an
old session continues to report its pinned deployment after the
`development` alias moves to a newer one. Each turn has a new `turnId`, each
assistant message a new `messageId`, and each call a new `toolCallId`; the
same tuple is on the call's [`tool.started`, `tool.progress`, `tool.completed`
and `tool.failed`](/agents/events#tools) events and, for the result tool, on
the session's [`result`](#the-session-result). The tuple identifies the
logical call: an invocation the platform retries keeps it, exactly as
`toolCallId` is kept above.

```tsx
export const submitProgress = defineTool({
  name: "submit_progress",
  description: "Record progress on the job",
  input: {
    type: "object",
  ...
  async run({ input, identity, signal }) {
    // identity is the platform's; input is the model's.
    await fetch("https://example.com/progress", {
      method: "POST",
      headers: { "Idempotency-Key": `${identity.sessionId}:${identity.toolCallId}` },
      body: JSON.stringify({ note: input.note, provenance: identity }),
      signal,
    });
...
- The value `run()` returns is validated against `output` and committed
  ...
  becomes `{ turnId, callId, reportedAt, data, identity }`, where `identity`
  is the committing call's [invocation identity](#invocation-identity), and
  the call's `tool.completed` event carries `result: true`
