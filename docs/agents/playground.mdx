...
- workspace files the agent wrote under `/workspace`, with verified download of
  a single file or the whole workspace as a zip (see
  [Workspace files](/agents/sessions#workspace-files))

Newly written workspace files appear gradually and may take a few moments to
finish syncing. Use the refresh action when a file has just been created.
