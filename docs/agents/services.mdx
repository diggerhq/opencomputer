...
description: "Call Google services, GitHub, and Linear with a credential the platform holds and refreshes"
---
...
| `linear` | `api.linear.app/graphql` | Linear |
...
Linear uses one GraphQL endpoint. Send a JSON GraphQL document with `POST`:
...
if (!response.ok || result.errors?.length) {
  throw new Error(result.errors?.[0]?.message ?? `linear: ${response.status}`);
}
```

Linear may report GraphQL errors in a successful HTTP response, so check both
...
| `provider` | `google`, `github`, or `linear` |
...
| | |
| --- | --- |
| Methods | `GET`, `POST`, `PUT`, `PATCH`, `DELETE`; Linear is `POST` only |
...
| --- | --- | --- |
| `404 connection_not_found` | No eligible connected account exists, or no account matches the requested `label` | `opencomputer connection list` to inspect the accounts, then connect one or correct the label |
