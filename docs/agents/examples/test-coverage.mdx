...
### Code Mode tools are unavailable

The workflow requires filesystem and shell capabilities after materialization
to inspect source, edit tests, and run validation commands. If a deployment
reports `No Code Mode tools are available`, stop the run and keep
`PUBLISH_ENABLED` set to `false`. Do not enable publishing until a complete dry
run has executed and observed the repository's test commands.
