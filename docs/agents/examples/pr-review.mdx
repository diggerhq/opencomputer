...
The [PR review example](https://github.com/diggerhq/opencomputer-example-pr-review)
...
and returns the review as session output. Only when the request explicitly
asks does it post a `COMMENT` review — a verdict plus findings, with inline
comments anchored to diff lines:

<Frame>
  <img
    src="/images/agents/pr-review-posted-review.png"
    alt="An inline review comment anchored to a diff line, posted by the agent with a concrete code suggestion"
  />
</Frame>

## What the example is made of

Three files carry the whole design.

**The agent function** — `opencomputer/agents/pr-review/agent.ts` — is a
synchronous render that runs before every model step. It reads the current
input and decides what the model gets for that step: the instructions and the
tool set. The write tool exists only when the request asks for a live post:
...
There is no separate permissions system: dry-run-by-default is one `if`
...
The connection is the agent's entire reach: tools call
...
**The managed runtime** supplies everything else: the durable session and
turn queue, the model/tool loop that calls the render before each step, and
