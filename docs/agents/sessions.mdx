...
### Retrying `session create` safely

`session create "<prompt>"` performs two changes: it creates the session and
then submits the first turn. Each has its own retry key, so a retried command
converges on one session and one turn instead of creating a second of either:

```bash
opencomputer --idempotency-key deploy-42 session create "Draft a response"
opencomputer session create "Draft a response" \
  --session-idempotency-key deploy-42-session \
  --turn-idempotency-key deploy-42-turn
```

With the global `--idempotency-key`, the CLI derives a separate session key
and turn key from the one value; they are only printed with `--verbose`. Retry
the same command unchanged after a lost response and it returns the session and
turn already committed. Retrying with a different prompt under the same turn
key, or different memory bindings under the same session key, is rejected as a
conflict rather than silently starting new work.

If the session is created but the first turn is not, the command fails with
`session_created_turn_failed` and the session ID, so the session is never
hidden. With `--json`, the error's `details` describe each step:

```json
{
  "session": { "id": "ses_01...", "created": true, "duplicate": false, "status": "running" },
  "turn": { "id": null, "admitted": false, "error": { "code": "...", "message": "..." } },
  "complete": false
}
```

Retry the same command with the same keys to converge on that session, or end
it with `opencomputer session end <session-id>`. A completed run reports the
same `session` and `turn` objects with `"complete": true`.

### Workspace files

Files an agent writes under `/workspace` (reports, captures, screenshots, tool
output) can be listed and downloaded with `session files`. Downloads never pass
through the model or the session transcript: OpenComputer exports the file into
provider-owned storage, records its byte count and SHA-256, and the CLI streams
that retained copy to disk, verifying both before the file is saved.
...
# Machine-readable output with artifactId, size and sha256 per file
...
temporary file and renamed into place only after the size and SHA-256 match
the export manifest, so an interrupted or corrupted transfer never leaves a
partial file behind. Retained copies remain downloadable after the session
ends, from the CLI, the session page's **Files** tab, or the
