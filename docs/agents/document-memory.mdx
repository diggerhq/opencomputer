...
session, `200` returned the one that key had already created. The call's
parameters and errors are in the
[Serverless Agents client reference](/reference/typescript-sdk/managed-agents#start-a-session-on-a-memory-document).
