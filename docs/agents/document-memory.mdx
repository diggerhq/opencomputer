...
- Bindings stay fixed. Schedules and webhooks do not configure them.
...
Reusing a session-create `Idempotency-Key` requires the same agent (or the
same pinned deployment), environment and bindings; incompatible inputs return
`409 idempotency_conflict`. The session keeps the deployment it started on,
so a redeploy does not change what the key returns. The
[management API](/agents/api#create) page has the full rules.
...
- The session is created with an `Idempotency-Key` derived from
  ...
  bindings returns the existing session; anything else under that key
...
The helper addresses the agent by its environment alias; the platform
records the deployment the session started on, so a retry after a redeploy
returns the same session. Do not change the key to recover a lost response:
that would create another session.
...
/api/managed-agents/sessions` with the document binding,
`agentId: <agent-id>@<environment>` and `Idempotency-Key`; `201` created the
