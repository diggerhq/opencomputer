...
In a project with several agents, `--agent <cloud-agent-id>` selects both the
cloud scope and the local agent whose connections supply the allowed origins,
so the command works from the project root; another agent's connections never
widen the secret. When the cloud id does not name one local agent, add
`--local-agent <local-agent-id>`; the error lists the valid local IDs and the
CLI never falls back to the first agent. The `--json` result includes the
resolved pair under `preflight.agent`:

```bash
printf %s "$TOKEN" | opencomputer secrets set API_TOKEN --value-stdin --agent app--billing --json
```

If the selected agent does not compile, the command fails before anything
changes in the cloud, with the same message `opencomputer doctor` and
`opencomputer deploy` report.
