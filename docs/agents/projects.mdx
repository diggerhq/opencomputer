...
If you skip linking, `opencomputer link` and `opencomputer deploy` ask you to
pick an existing project or name a new one when run in an interactive
terminal. Without a terminal (CI, pipes, `--json`), project-scoped commands
return a structured `binding_required` error with the explicit command to run.
