...
| `toolCalls` | `{ callId, tool, title, input?, output?, status }` in the order the calls started; `status` is `running`, `completed` or `failed` |
| `result` | The output of the agent's result tool, when it reported one; a later call replaces it |
| `failure` | `{ code, message }` from `turn.failed` |

Tool calls are keyed on the runtime's `callId`, so a `tool.completed` updates
the entry its `tool.started` created rather than adding one. An activity
timeline renders `turns` directly:
