...
flowchart LR
    ...
    B --> C["Local agent server<br/>npm run dev"]
...
`npm run dev` starts the agent server. It writes ephemeral connection details
inside the selected agent's ignored `.opencomputer/` directory. `npm run
dev:web` starts Vite; its server-side proxy reads those details and forwards
browser requests. The credential is never bundled into React code.
