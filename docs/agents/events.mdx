...
| Type | When | `data` |
| --- | --- | --- |
| `tool.started` | The model called a tool | `tool`: its name; `callId`; `title`; `input`: the arguments as a JSON value; `identity` |
| `tool.progress` | The tool reported progress | `callId`, `identity`, and the metadata the tool reported |
| `tool.completed` | The tool returned | `tool`, `callId`, `title`, `output`: what the tool returned as a JSON value; `result: true` when the output became the session's result; `identity` |
| `tool.failed` | The tool raised an error, or its turn ended before it completed | `tool`, `callId`, `title`, `message`, `identity`; `settledBy` when the turn's end settled the call: `turn.completed`, `turn.failed` or `turn.cancelled` |
...
Every `tool.*` event carries `identity`: the same eight fields the tool
itself received as its [invocation identity](/agents/tools#invocation-identity),
assigned by OpenComputer from the session and turn it admitted:
...
```json
{
  ...
  "turnId": "turn_01…",
  "data": {
    "tool": "submit_progress",
    "callId": "call_01…",
    "title": "submit_progress",
...
The four events of one call carry the same tuple, `identity.toolCallId` is
the event's `callId`, and `identity.turnId` is the event's `turnId`. The
session stamps it when it records the event; a value the tool or the model
placed in `input` under the same name stays in `input`, as data, and never
replaces it. A `tool.failed` the session records for a call still open when
its turn ended carries the identity of the `tool.started` it settles. The
tuple stays in the log after the session ends, and the same tuple is on the
session's `result` for the call that committed it. Tool events recorded
before `identity` was introduced do not have it; read it as optional.
