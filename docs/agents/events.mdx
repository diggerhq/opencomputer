...
`model_rejected` at once, except that a managed model refused for want of
OpenComputer credit is `balance_exhausted`. A `model_stream_failed` message
names its retry only when one happened.
...
| `balance_exhausted` | The organization's OpenComputer balance cannot fund a managed model call; top up to continue | |
