...
| Type | When | `data` |
| --- | --- | --- |
| `session.created` | The session exists. Always the first event. | `agentId`, `deploymentId`; `sessionData`: `{ schemaId?, bytes, digest, revision }` describing the [session data](/agents/session-data) the session was created with, when there was any. The values themselves are not in the event. |
...
| Type | When | `data` |
| --- | --- | --- |
| `message.received` | A turn was accepted; its input is recorded before anything runs | `input`: the user text, empty for a payload-only turn; `mode`: `queue`, `steer` or `interrupt`; `payload`: `{ schemaId?, bytes, digest }` describing the structured value the turn was sent with, when there was one. The value itself is on the turn in `GET /sessions/<id>`, not in the event. |
...
succeed, and there is one per call. Each commit is also kept as an immutable
record on the [results routes](/agents/results), keyed by the turn and the
`callId`.
