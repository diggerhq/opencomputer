...
missing secret declarations, and then prints the project it resolved and the
cloud id each local agent deploys as. Run `opencomputer doctor --json` directly
to get structured diagnostics, with the same resolution under `resolution`,
before invoking a deployment agent.
