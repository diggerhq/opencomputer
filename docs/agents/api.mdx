...
| `sessionData` | Optional. A JSON object the agent reads with [`useSessionData(key)`](/agents/session-data), fixed for the life of the session: at most 32 KiB of JSON, 32 levels deep, keys of at most 200 characters. Never shown to the model by the platform. |

The response is `{ session: { id, executionMode, status, createdAt }, deployment }`.
A session created with `sessionData` also carries `sessionDataDigest`, the
`sha256:` digest of the data with keys in sorted order, and
`sessionDataRevision`, which is `1`. The session starts without a turn; send
one with the turns route.
...
- The same key with the same request, meaning the same `agentId` (or the
  same pinned `deploymentId`), environment, memory bindings and
  `sessionData` (key order does not matter), returns the existing session
  with `200`, together with the deployment it pinned when
...
- Anything else under that key is `409 idempotency_conflict`: another
  agent, environment or pinned deployment, different memory bindings, or
...
`400 invalid_labels`, `400 invalid_session_data` (not an object, over the
limits, or a key that is empty or reserved), `404 deployment_not_found`, `409 deployment_not_promoted`
...
| `turns` | Every turn: `id`, `input`, `mode`, `status`, `createdAt`, `updatedAt`, `payload` when the turn was sent with one, and `deliveries` when an [event subscription](#event-subscriptions) selected its outcome |
| `result` | The latest committed output of the agent's [result tool](/agents/tools#the-session-result), or `null`: `{ turnId, callId, reportedAt, data }`. `turnId` and `callId` name the call that reported `data`, so a reader can tell which turn it came from and whether a later turn changed things without reporting. A failed or cancelled turn never clears it. Every committed result, not only the latest, is on the [results routes](#results). |
| `sessionDataDigest`, `sessionDataRevision` | Present when the session was created with [`sessionData`](/agents/session-data): its digest and revision (`1`) |
...
| Field | Meaning |
| --- | --- |
| `input` | The user text for this turn. Required unless a non-empty `payload` is sent; a turn with neither is `400 invalid_turn`. |
...
| `payload` | Optional. A JSON value for the agent, at most 32 KiB encoded and 32 levels deep. It reaches the agent as [`useInput().payload`](/agents/inputs) with `source: "user"`, exactly as sent, next to the text. The text stays the message; the payload is structured context the agent reads, and the platform never renders it into the model's instructions. A payload-only turn tells the model that structured input arrived without a message. |

The turn's `message.received` event records the payload's schema id (the
top-level `schema` string, when the payload is an object with one), byte count
and digest, not the payload. The payload itself is on the turn in
`GET /sessions/<id>` and in the agent's input.
...
Codes: `400 invalid_turn`, `400 invalid_payload` (over 32 KiB or deeper than
32 levels),
...
Every value the agent's [result tool](/agents/tools#the-session-result)
commits is kept as an immutable record owned by the turn that committed it.
[Results](/agents/results) explains the record and its guarantees; the routes:

| Operation | Method and path | Success |
| --- | --- | --- |
| List | `GET /sessions/<id>/results` | `200` with `{ results, nextCursor }` |
| List for a turn | `GET /sessions/<id>/turns/<turn-id>/results` | `200` with `{ results, nextCursor }`; `results: []` when the turn committed none |
| Get | `GET /sessions/<id>/results/<result-id>` | `200` with the record |

```bash
curl 'https://app.opencomputer.dev/api/managed-agents/sessions/<session-id>/results?limit=50' \
  -H 'x-api-key: <api-key>'
```

| Parameter | Meaning |
| --- | --- |
| `limit` | Records per page, at most 200 (a larger value returns 200); default 50 |
| `cursor` | The `nextCursor` of the previous page |

Records are ordered by commit, oldest first, and never move; a page is
`{ results, nextCursor }` with `nextCursor: null` at the end. Codes: `400
invalid_limit`, `400 invalid_cursor` (a cursor not issued by this listing),
`404 result_not_found`, and `404 session_not_found` for a session another
project or environment owns.
