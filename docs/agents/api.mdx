...
| `result` | The latest committed output of the agent's [result tool](/agents/tools#the-session-result), or `null`: `{ turnId, callId, reportedAt, data, identity }`. `turnId` and `callId` name the call that reported `data`, so a reader can tell which turn it came from and whether a later turn changed things without reporting; `identity` is that call's full [invocation identity](/agents/tools#invocation-identity) (project, environment, agent, deployment, session, turn, message and call), absent only on a result committed before it was recorded. A failed or cancelled turn never clears it. |
