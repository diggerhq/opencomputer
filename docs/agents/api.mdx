...
| Capability manifest | `GET /deployments/<id>/capabilities` | `200` with `{ manifest, manifestDigest }`; the digest is also the `ETag`. See [Capability manifest](/agents/deployments#capability-manifest) |
