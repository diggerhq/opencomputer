...
`GET /sessions/<id>/events?after=<seq>` returns `{ events, cursor,
session, turn, waitExpired }`: up to 500 events with `seq` greater than
`after`, in ascending order, with the page's position and the terminal
state of the session and its current turn. Each event is
`{ id, seq, timestamp, sessionId, turnId, type, data }`; `turnId` is absent
on session-level events.

| Query | Meaning |
| --- | --- |
| `after` | Return events with a greater `seq`; default `0` |
| `wait` | Seconds to hold the request for an event newer than `after`, at most `30`; default `0`. An expired wait is `200` with `waitExpired: true` |
| `limit` | Events per page, `1` to `1000`; default `500` |
| `turn` | The turn `turn` describes; default the running turn, else the most recent |

To read a whole log, start at `after=0` and repeat with `cursor.nextAfter`
until it equals `cursor.highWatermark`. To follow a live session, read with
`wait` from the last cursor and stop when `session.terminal` (or the turn
you follow) is true; the CLI's `sessions tail` and `useAgent` follow the
log this way. The log is durable, so a consumer that stops can resume from
its cursor and miss nothing. At most 32 requests may wait on one session
at once; a further one is `429 too_many_waiters`. Field meanings and
event types are on [Session events](/agents/events#waiting-for-the-next-event).
...
in a project to a destination: a session in the same project, as a new turn
of that session, or an HTTPS endpoint of your application, as a signed
request. The first is how one agent learns that another finished: a
coordinator subscribes to its workers' outcomes and reasons about them when
they arrive. The second is described on
[Event delivery](/agents/event-delivery); this is not the
[agent webhooks](/agents/webhooks) feature, which is inbound.

| Operation | Method and path | Success |
| --- | --- | --- |
| Create | `POST /projects/<p>/event-subscriptions` | `201` with `{ subscription }`, plus `signingSecret` once for an HTTPS destination |
...
| Pause, resume | `POST /projects/<p>/event-subscriptions/<id>/pause`, `.../resume` | `200` with `{ subscription }`; HTTPS destinations only |
| Rotate secret | `POST /projects/<p>/event-subscriptions/<id>/rotate-secret` | `200` with `{ subscription, signingSecret }`; HTTPS destinations only |
| Deliveries | `GET /projects/<p>/event-subscriptions/<id>/deliveries` | `200` with `{ deliveries, nextCursor? }`; HTTPS destinations only |
| Delivery | `GET /projects/<p>/event-subscriptions/<id>/deliveries/<deliveryId>` | `200` with `{ delivery }` |
| Replay | `POST /projects/<p>/event-subscriptions/<id>/replay` | `202` with `{ deliveries }` |
...
| `destination` | `{ type: "session", sessionId }`: a session of an agent in this project that can still take turns. Or `{ type: "https", url }`: an absolute `https://` URL without credentials or fragment; see [Event delivery](/agents/event-delivery). |
| `environment` | `development` or `production`. A subscription is scoped to one environment: it selects outcomes recorded there, and its destination receives only those. |

A subscription's selection and destination are immutable; `{ subscription }`
carries `id`, `projectId`, the fields above, `createdAt` and, for an HTTPS
destination, `status` (`active` or `paused`), `updatedAt` and
`secretRotatedAt`. Codes: `400 invalid_event_subscription`,
...
`404 event_subscription_not_found`, `404 event_delivery_not_found`,
`400 invalid_deliveries_query`, `400 invalid_replay`.
...
For an HTTPS destination, the entry in the source session's `deliveries`
records that the event was handed to delivery and carries `deliveryId`, the
id of the record the subscription's `deliveries` routes track through its
attempts; see [Event delivery](/agents/event-delivery#delivery-state).
