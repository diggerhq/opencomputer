...
| `runtime` | The computer serving the session: `generation`, `state`, `startedAt`, `expiresAt`, `releasedAt`, `releaseReason`, `replacedAt`, `replacementReason`, `lifetimeSeconds`, `idleReleaseSeconds`; see [Session lifecycle](/agents/lifecycle#observing-the-computer) |
| `retention` | The effective retention: `sessionExpiresAt`, `workspaceExpiresAt`, `eventHistoryExpiresAt` (`null` is no time-based expiry while the project exists) and `policyVersion`; see [Retention](/agents/lifecycle#retention) |
...
`POST /sessions/<id>/suspend` releases the session's computer between turns
and keeps the session resumable; the next turn acquires a new computer over
the same workspace. It returns the session with `compute: { released,
generation }`. `409 turn_in_progress` while a turn is running or queued;
`409 session_ended` for an ended session. Idle release happens without this
call; see [Session lifecycle](/agents/lifecycle#releasing-a-computer-explicitly).

Sessions created with `executionMode: "microvm"` hibernate their computer
instead: the session passes through `suspending` to `suspended`, and
`POST /sessions/<id>/resume` wakes the same computer.
