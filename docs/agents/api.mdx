...
| `networkPolicy` | A deny-by-default list of the exact HTTP and HTTPS origins the session's computer may reach, enforced beneath every process and immutable for the session; see [Network egress policy](/agents/network-policy). Part of the request identity under `Idempotency-Key`. |
...
- Anything else under that key is `409 idempotency_conflict`: another
  ...
  different (or added or removed) `networkPolicy`.
...
`400 invalid_labels`, `400 invalid_network_policy`, `404 deployment_not_found`,
`409 deployment_not_promoted` (a pinned deployment is not active in the named
environment), `409 egress_policy_unsupported` (a `networkPolicy` on a session
whose commands do not run in a managed computer), `402 insufficient_credits`.
...
| `networkPolicy` | The [network policy receipt](/agents/network-policy#receipt), with `policyDigest` and `state`; present when the session was created with a policy |
...
`POST /sessions/<id>/network-policy/revoke` revokes the session's
[network policy](/agents/network-policy#revocation): every new connection
from its computer is denied and open connections are closed, without
waiting for the running turn. The body may carry a `reason`. The response
is `{ networkPolicy, changed, enforcement }`, where `enforcement.closed`
says whether the computer's egress is confirmed closed. A session without a
policy answers `404 egress_policy_required`.
