...
| Capability manifest | `GET /deployments/<id>/capabilities[?digest=sha256:...]` | `200` with `{ manifest, manifestDigest }`; the digest is also the `ETag`. `digest` selects a manifest the deployment carried earlier (`404 manifest_not_found` if unknown). See [Capability manifest](/agents/deployments#capability-manifest) |
| Readiness | `POST /deployments/<id>/readiness` | `200` with a [readiness receipt](/agents/deployments#readiness-receipt); `ready: false` is still `200` |
