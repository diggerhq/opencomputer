...
In a single-environment project, a schedule that enables `production` (or
`default`) runs automatically; one that enables only `development` is
**Manual only**.

### Legacy Development and Production projects
...
sessions, secrets, and database state.
