...
`agent.ts`. Loading a skill lists them, and the agent reads one on request —
so a long checklist or a set of examples costs nothing until it is needed.

An agent can only read files packaged with the skill it loaded. There is no
path out of that directory, and no way to reach anything else on the machine.
