...
A **render** is one call of this function. OpenComputer renders the agent
before each model step of a turn, so a single turn typically produces several
renders, and each one can return different instructions or attach different
capabilities depending on the input and session state it reads. See
[How it works](/agents/mental-model#a-render-is-one-call-of-your-agent-function)
