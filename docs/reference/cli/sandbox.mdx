...
Show which [secret store](/sandboxes/secrets) is attached to a sandbox, the names of the secret environment variables it injects (values are never returned), and the egress allowlist + per-secret allowed hosts the sandbox's secrets proxy enforces. Useful for debugging "why is my outbound HTTP call being blocked" without having to cross-reference the store config separately. The same information is shown on the sandbox detail page in the dashboard.
