...
- **Spot sandboxes** are fixed at 1 vCPU / 1 GB during alpha. `scale()` and `set_autoscale(enabled=True)` raise `SandboxFamilyLimitError`.
...
```python
from opencomputer import Sandbox, ScalingLockedError, PlanLimitError, SandboxFamilyLimitError
...
Raised when the sandbox's family does not allow resource changes. Alpha [spot sandboxes](/sandboxes/spot-sandboxes) are fixed at 1 vCPU / 1 GB, so `scale()` and `set_autoscale(enabled=True)` raise this error.

```python
from opencomputer import SandboxFamilyLimitError
