...
| `image` | object | No | Declarative image manifest (see [Image builder](/sandboxes/templates#image-configuration)) |
| `snapshot` | string | No | Name of a pre-built snapshot (see [Snapshots](#snapshots)) |
...
```bash
curl -X POST https://app.opencomputer.dev/api/sandboxes/{id}/exec/run \
  -H "X-API-Key: $OPENCOMPUTER_API_KEY" \
  -H "Content-Type: application/json" \
...
Pass auth via query parameter: `?api_key=<key>`
...
```bash
curl -X POST https://app.opencomputer.dev/api/sandboxes/{id}/agent \
  -H "X-API-Key: $OPENCOMPUTER_API_KEY" \
  -H "Content-Type: application/json" \
...
```bash
curl https://app.opencomputer.dev/api/sandboxes/{id}/files?path=/app/index.js \
  -H "X-API-Key: $OPENCOMPUTER_API_KEY"
```
...
```bash
curl -X PUT https://app.opencomputer.dev/api/sandboxes/{id}/files?path=/app/hello.txt \
  -H "X-API-Key: $OPENCOMPUTER_API_KEY" \
...
Snapshots are pre-built sandbox environments created from declarative image manifests. Once built, sandboxes can be created from them instantly.
...
| `image` | object | Yes | Declarative image manifest (see [Image builder](/sandboxes/templates#image-configuration)) |
...
Status transitions: `building` → `ready` or `failed`.

### `GET /api/snapshots` — List

**Response** `200`: Array of snapshot objects.

### `GET /api/snapshots/:name` — Get

**Response** `200`: Snapshot object.

### `DELETE /api/snapshots/:name` — Delete

**Response** `204`: No content.

### Using snapshots and images with `POST /api/sandboxes`

The create sandbox endpoint accepts `snapshot` or `image` fields in addition to `templateID`:

| Field | Type | Description |
| --- | --- | --- |
| `snapshot` | string | Name of a pre-built snapshot — instant boot, no build step |
| `image` | object | Declarative image manifest — built on first use, cached by content hash |

```bash
# From a snapshot (instant)
curl -X POST https://app.opencomputer.dev/api/sandboxes \
  -H "X-API-Key: $OPENCOMPUTER_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"snapshot": "data-science", "timeout": 600}'

# From a declarative image (built + cached)
curl -X POST https://app.opencomputer.dev/api/sandboxes \
  -H "X-API-Key: $OPENCOMPUTER_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"image": {"steps": [{"type": "apt_install", "packages": ["curl"]}]}, "timeout": 600}'
```
