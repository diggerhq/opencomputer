...
The management API client for Serverless Agents is a separate, portable
subpath of the same package, `@opencomputer/sdk/managed-agents`, with the
memory and event subscription types and `startSessionOnDocument`. It is
documented on its own page:
[Serverless Agents client](/reference/typescript-sdk/managed-agents).
