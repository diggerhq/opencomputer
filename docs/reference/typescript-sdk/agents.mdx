...
otherwise. A `429` also carries `retryAfter` in seconds, an error the API
tied to a session carries `sessionId`, and a workspace artifact export error
carries `retrySafe`, whether the unchanged request can be resubmitted under a
new idempotency key.
...
| Method | Route | Returns |
| --- | --- | --- |
| `sessions.artifacts.export({ sessionId, path, mediaType?, expected?, idempotencyKey })` | `POST /sessions/<id>/workspace-artifacts/exports` | `{ export, created }`; `created` is `false` when the key had already requested it |
| `sessions.artifacts.get(exportId)` | `GET /workspace-artifact-exports/<id>` | The `WorkspaceArtifactExport` |
| `sessions.artifacts.list(sessionId)` | `GET /sessions/<id>/workspace-artifacts/exports` | The session's exports |
| `sessions.artifacts.cancel(exportId)` | `POST /workspace-artifact-exports/<id>/cancel` | The export; unchanged when already terminal |
| `sessions.artifacts.waitUntilTerminal(exportId, { signal?, pollIntervalMs? })` | polls `get` | The export once `delivered`, `failed`, `cancelled` or `expired` |
| `sessions.artifacts.download(exportId, { verify?, signal? })` | `GET /workspace-artifact-exports/<id>/content` | `{ stream, bytes, sha256, mediaType, artifactId, exportId }` |

`download` returns the response body as a `ReadableStream<Uint8Array>`
without buffering it, with the size, digest and ids from the response
headers. By default (`verify: true`) the stream hashes and counts the bytes
as they pass and fails with `WorkspaceArtifactIntegrityError`, whose `code`
is `artifact_digest_mismatch` or `artifact_size_mismatch`, when they do not
match the headers; discard what you wrote when that happens. Aborting the
signal given to `waitUntilTerminal` rejects with its reason and leaves the
export running. See [Workspace artifact exports](/agents/artifacts) for the
record, states, error codes and idempotency rules.
...
exported from the same place, as are the artifact export types
(`WorkspaceArtifactExport`, `WorkspaceArtifactExportState`,
`WorkspaceArtifactExportErrorCode`, `WorkspaceArtifactExportError`,
`CreateWorkspaceArtifactExportParams`, `WorkspaceArtifactDownload`) with
`isWorkspaceArtifactExportTerminal(state)` and
