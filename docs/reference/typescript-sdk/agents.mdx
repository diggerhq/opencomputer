...
the caller decides which calls to repeat and with which key. The one error
that asks for a retry, `503 session_publication_unconfirmed` from
`sessions.create` or `sessions.setLabels`, is thrown like any other: the
session exists, or its labels are recorded, but its row in the list was not
confirmed in time, and `error.sessionId` names it. Repeat the same call, with
the same `idempotencyKey` and params on `create` or the same patch on
`setLabels`, until it returns; a return means the list reflects the change
([Management API](/agents/api#create)).

```typescript
for (;;) {
  try {
    return await oc.sessions.create(params, { idempotencyKey: taskId });
  } catch (error) {
    if (!(error instanceof OpenComputerError) || error.code !== "session_publication_unconfirmed") throw error;
