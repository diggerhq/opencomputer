...
| `sessions.list(query?)` | `GET /sessions` | `{ sessions, nextCursor }`: one page |
| `sessions.iterate(query?)` | `GET /sessions`, page by page | An async iterator over every matching row |
...
`labels` and `externalReference` as [documented](/agents/api#create). `turns.send` takes `input`,
...
after a lost reply learns what became of the turn. `list` takes the exact
filters of [`GET /sessions`](/agents/api#get-and-list): `project` or
`projectId`, `environment`, `agent` or `agentId`, `status`, `deploymentId`,
`externalReference`, `createdAfter`, `createdBefore`, `updatedAfter`,
`labels` (up to three, sent as `label.<key>=<value>`), plus `cursor` and
`limit` (1 to 100, default 50). `iterate` takes the same query and requests
the next page whenever the previous one is consumed, stopping when
`nextCursor` is `null`; it reads live rows, so a session created while you
iterate may or may not be seen.

```typescript
const recovered = await oc.sessions.list({ externalReference: orderId });
for await (const row of oc.sessions.iterate({ status: "suspended", limit: 100 })) {
