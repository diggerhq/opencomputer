...
Create a new sandbox. [HTTP API →](/v2/api-reference/sandboxes/create)
...
<ParamField body="burst" type="boolean">
  Create a [Burst Sandbox](/v2/sandboxes/burst-sandboxes). Disk is preserved across infrastructure restarts; processes may restart.
</ParamField>
...
<ParamField body="image" type="Image">
  Declarative image definition (see [Image](/v2/reference/typescript-sdk/image))
</ParamField>
...
Connect to an existing sandbox. [HTTP API →](/v2/api-reference/sandboxes/get)
...
Create a new sandbox from a checkpoint. [HTTP API →](/v2/api-reference/checkpoints/fork)
...
Create a patch for a checkpoint. [HTTP API →](/v2/api-reference/patches/create)
...
[HTTP API →](/v2/api-reference/patches/list)
...
[HTTP API →](/v2/api-reference/patches/delete)
...
Terminate the sandbox. [HTTP API →](/v2/api-reference/sandboxes/delete)
...
Snapshot VM state and stop. No compute cost while hibernated. [HTTP API →](/v2/api-reference/sandboxes/hibernate)

**Returns:** `Promise<void>`

### `sandbox.wake(opts?)`

Resume a hibernated sandbox. [HTTP API →](/v2/api-reference/sandboxes/wake)
...
Update the idle timeout. [HTTP API →](/v2/api-reference/sandboxes/set-timeout)
...
Create a named checkpoint. [HTTP API →](/v2/api-reference/checkpoints/create)
...
[HTTP API →](/v2/api-reference/checkpoints/list) — **Returns:** `Promise<CheckpointInfo[]>`

### `sandbox.restoreCheckpoint(checkpointId)`

Revert in-place to a checkpoint. [HTTP API →](/v2/api-reference/checkpoints/restore)

**Returns:** `Promise<void>`

### `sandbox.deleteCheckpoint(checkpointId)`

[HTTP API →](/v2/api-reference/checkpoints/delete) — **Returns:** `Promise<void>`
...
Generate a signed download URL. [HTTP API →](/v2/api-reference/files/generate-download-url) · [Guide →](/v2/sandboxes/signed-urls)
...
Generate a signed upload URL. [HTTP API →](/v2/api-reference/files/generate-upload-url) · [Guide →](/v2/sandboxes/signed-urls)
...
[HTTP API →](/v2/api-reference/preview/create)
...
[HTTP API →](/v2/api-reference/preview/list) — **Returns:** `Promise<PreviewURLResult[]>`

### `sandbox.deletePreviewURL(port)`

[HTTP API →](/v2/api-reference/preview/delete) — **Returns:** `Promise<void>`

---

### `sandbox.getAllowedHosts()`

Return the egress allowlist + per-secret allowed hosts the sandbox's secrets proxy enforces. Useful for debugging "why is my outbound HTTP call being blocked" without having to cross-reference the [secret store](/v2/sandboxes/secrets) config separately.
...
<ParamField body="exec" type="Exec">
  [Command execution](/v2/reference/typescript-sdk/exec)
</ParamField>

<ParamField body="files" type="Filesystem">
  [File operations](/v2/reference/typescript-sdk/filesystem)
</ParamField>

<ParamField body="pty" type="Pty">
  [Terminal sessions](/v2/reference/typescript-sdk/pty)
</ParamField>
