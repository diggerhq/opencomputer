...
Run a command synchronously via `sh -c`. [HTTP API →](/v2/api-reference/exec/run)
...
Start a long-running command with streaming I/O. [HTTP API →](/v2/api-reference/exec/create-session)
...
Kill an exec session. [HTTP API →](/v2/api-reference/exec/kill-session)
