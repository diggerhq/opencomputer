...
See [Templates](/v2/sandboxes/templates) for the full guide.

## Elasticity

Sandboxes can scale memory at runtime via an internal metadata endpoint (`http://169.254.169.254/v1/scale`) accessible from inside the VM. CPU scales proportionally. This lets workloads request more resources before heavy tasks and release them after. See [Elasticity](/v2/sandboxes/elasticity) for details.
...
See [Preview URLs](/v2/sandboxes/preview-urls) for custom domains and auth.
