...
inside an OpenComputer sandbox. Use [sandboxes](/v2/sandboxes/overview) when you
...
not provide a live view. Telemetry is enabled by default for all browser
sessions. Set `telemetry: false` / `telemetry=False` to disable it, or pass a
