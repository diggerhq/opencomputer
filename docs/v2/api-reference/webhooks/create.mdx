...
Register a webhook destination for sandbox lifecycle events. See [Webhooks](/v2/sandboxes/webhooks).
...
<ParamField body="secret" type="string">
  Signing secret. Omit and one is generated (`whsec_…`). The secret is returned in the create response and is re-fetchable any time via [`GET /api/webhooks/{id}/secret`](/v2/api-reference/webhooks/get).
</ParamField>
...
The `secret` is returned here and stays re-fetchable via [`GET /api/webhooks/{id}/secret`](/v2/api-reference/webhooks/get) — store it to verify deliveries.
</ResponseExample>
