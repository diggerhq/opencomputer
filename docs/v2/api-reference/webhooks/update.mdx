...
Update a destination — pause/resume, retune filters, change the URL, or rotate the secret. See [Webhooks](/v2/sandboxes/webhooks).
...
`sandboxId` (scope) is **immutable** — set it at create; it can't be changed here. The current signing secret is re-fetchable any time via [`GET /api/webhooks/{id}/secret`](/v2/api-reference/webhooks/get).
