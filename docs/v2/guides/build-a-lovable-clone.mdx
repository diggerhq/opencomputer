...
<Note>
For new app-builder projects, start with [Durable Agent Sessions](/agent-sessions/quickstart) and the [app-builder example](https://github.com/diggerhq/oc-sessions-demos). This guide shows the older raw-sandbox orchestration pattern, which is still useful when you want to own the agent loop yourself.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Durable sessions example" icon="github" href="https://github.com/diggerhq/oc-sessions-demos">
    The recommended app-builder architecture for new projects.
  </Card>
...
- **In-sandbox agent loop** — [Agent sessions](/agent-sessions/overview) now run the Claude Agent SDK inside the sandbox. The agent reads files, runs commands, and iterates on errors locally without leaving the VM. This is the recommended approach for new projects.
- **Preview URL authentication** — `authConfig` on [preview URLs](/v2/sandboxes/preview-urls) supports restricting access. Not full private link sharing yet, but a step toward it.
- **Checkpoints** — [Checkpoint and fork](/v2/sandboxes/checkpoints) workflows are available for snapshotting sandbox state, restoring, and branching.
- **Custom templates** — Define [custom environments](/v2/sandboxes/templates) with the Image builder and pre-built snapshots.
