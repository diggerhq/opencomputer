...
Browser setup is heavy (apt packages + Chromium binary is ~500MB). Bake it into a [template](/v2/sandboxes/templates) once, launch sandboxes from it in seconds.
...
The profile lives on the data disk, which survives sandbox hibernation. For cross-sandbox persistence, [checkpoint the sandbox](/v2/sandboxes/checkpoints) after login and launch future sandboxes from that warm snapshot — they'll boot already logged in.
...
- See [Checkpoints](/v2/sandboxes/checkpoints) for how to checkpoint a warmed-up browser VM.
- See [Secret Stores](/v2/sandboxes/secrets) for scoping egress and sealing real credentials.
