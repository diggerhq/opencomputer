...
The SDK ships a verifier — no extra dependency, runs in Node / Cloudflare Workers / browsers — that checks the signature and returns the parsed envelope. Pass `SandboxLifecycleEvent` to type the nested event:
