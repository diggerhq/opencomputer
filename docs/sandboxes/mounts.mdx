...
<Warning>
  **Mounts do not survive hibernate.** When a sandbox hibernates (manually or
  via idle timeout), all FUSE mounts are torn down before the snapshot is
  taken — otherwise the snapshot would capture broken file descriptors that
  hang processes on wake.

  v1 does not auto-restore mounts on wake. After waking, call `mounts.add()`
  again for anything you need back. `mounts.list()` returns empty post-wake.
</Warning>

The simplest pattern is to remount on wake:
