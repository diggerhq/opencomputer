...
```
POST /api/sandboxes/:id/files/download-url
```
...
| `expiresIn` | number | 3600 | URL lifetime in seconds (max: 86400) |

**Response:**
```json
{
  "url": "https://app.opencomputer.dev/api/sandboxes/sb-xxx/files/download?path=...&expires=...&signature=...",
...
```
POST /api/sandboxes/:id/files/upload-url
```

Same request/response format as download URL.
...
No authentication required. Returns the file content with `Content-Length` header.
...
No authentication required. Send file content as the request body. Returns `204` on success.
