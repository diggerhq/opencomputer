...
<ParamField body="rotateSecret" type="boolean">
  Generate a **new** signing secret. The new `whsec_…` is returned **once** in the response (like create). Use this instead of `secret` when you want OpenComputer to mint the secret.
</ParamField>
...
`sandboxId` (scope) is **immutable** — set it at create; it can't be changed here.
