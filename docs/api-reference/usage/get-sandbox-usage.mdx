...
<ParamField query="from" type="string">
  Lower bound. Accepts either a date (`2026-05-27`, treated as UTC
  midnight) or a full RFC3339 timestamp. Default: now minus 1 hour.
</ParamField>

<ParamField query="to" type="string">
  Upper bound. Same format as `from`. Default: now. Window must be ≤
  30 days.
</ParamField>
...
To rank sandboxes by spend, use [`GET /api/usage`](/api-reference/usage/get-usage)
