...
<Note>
  `memoryMB` is the only opportunity to size a sandbox: the size is fixed for the
  sandbox's lifetime and cannot be changed afterwards. A size outside the
  supported tiers is rejected rather than rounded down, so you are never quietly
  given less than you asked for. See [Sizing](/sandboxes/sizing).
</Note>
