...
async def main():
    ...
    # getattr, not sandbox.end_at: the 0.x SDK has no such attribute, so a
    # direct read raises AttributeError instead of reporting v1.
    end_at = getattr(sandbox, "end_at", None)
    print("version:", "v2" if end_at else "v1")
    if end_at:
        print("endAt =", end_at)
