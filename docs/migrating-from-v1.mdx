...
No code changes are required to upgrade. Your API key, your calls, and your existing sandboxes
all keep working. (`1.1.0` is the floor because that is where `endAt` — used by the check below
— was added; any `1.x` routes to v2.) Read [Am I affected?](#am-i-affected) before you ship it to production — a
handful of behaviours differ, and two of them can fail a workload that never checked.
