...
Inline deployments finish synchronously (`state: "ready"` with a `revision_id`). Asynchronous deployments (GitHub) return `state: "queued"` — poll `GET /agents/:id/deployments/:deployment_id`; a repo deploy also reports progress as a **commit status** on GitHub.
