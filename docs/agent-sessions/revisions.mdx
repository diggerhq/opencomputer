...
From a deploy's point of view, an agent **is a small directory**:
...
`agent.toml` + `prompt.md` are all an agent needs; `skills/` and `runtime/` are additive — add them only when you do. A **deploy** packages this directory into an immutable **revision**; the **active revision** is what new [sessions](/agent-sessions/sessions) use, and you roll back by re-activating an earlier one.
...
Keep the agent directory (above) in Git and **push to deploy**. Install the OpenComputer GitHub App on the repo (the agent page in the [dashboard](https://app.opencomputer.dev), or **Settings → GitHub**), then connect it to an agent:
