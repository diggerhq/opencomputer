...
Narrow edges, all on the roadmap (see [Failure behavior](#failure-behavior)): a sandbox lost **mid-turn** re-runs that turn; a hung turn ends at its deadline rather than auto-continuing; **uncommitted files** don't survive sandbox loss (commit/push to keep them).

## Durable event log

Every step is appended with a monotonic `seq`. Reads and live streams are served from that ordered log, so anything logged or delivered is recoverable.
