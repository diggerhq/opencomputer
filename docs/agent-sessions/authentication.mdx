...
By default, runs are **platform-billed on Anthropic** — zero setup, metered to your org, capped per session. To run on **your own** provider key, add a credential once. It's stored in our secret store (encrypted, write-only, never returned) — you never manage a raw secret store yourself.
...
**Resolution** for each run: the agent's credential → your org default for the model's provider → platform billing.

<Note>Today only **Anthropic** (the `claude` runtime). Other providers and model hot-swap are [coming soon](/agent-sessions/runtimes) — the credentials shape is the same, so adding them won't break anything.</Note>
