...
Sessions run the model on **your own provider key**: Anthropic for the `claude` [runtime](/agent-sessions/runtimes), OpenAI for `codex`. The key is held in OpenComputer's [secret store](/sandboxes/secrets), encrypted at rest (AES-256-GCM), write-only, and **never returned** by the API. The real key never enters a sandbox: the runtime gets an opaque placeholder, and a host-side proxy swaps in the real value only on the outbound HTTPS call to the model provider (`api.anthropic.com` or `api.openai.com`, scoped by an egress allowlist). A credential is **required**; a session with no resolvable key fails to start (`422 no_credential`).

The easiest way to add one is inline when you [create an agent](/agent-sessions/agents) (`"key": "…"`) — the provider is inferred from the agent's `model`. The standalone resource below is for reusing one key across agents, setting an org default, and rotating/removing keys. Set `provider` to `anthropic` or `openai`:
...
**Resolution** per session: the agent's credential → your org default for the provider. Sessions run on your configured provider key.
