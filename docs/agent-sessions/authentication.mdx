...
<Tip>Examples that call `oc()` use the server-side helper from the [quickstart](/agent-sessions/quickstart#1-add-the-server-helper).</Tip>
...
```ts
const res = await fetch("https://api.opencomputer.dev/v3/sessions", {
  method: "POST",
  headers: {
...
```ts
const token = await oc(`/sessions/${sessionId}/client-tokens`, {
  method: "POST",
...
```ts
const credential = await oc<{ id: string }>("/credentials", {
  method: "POST",
  body: JSON.stringify({
    provider: "anthropic",
