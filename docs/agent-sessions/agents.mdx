...
<Note>The [dashboard](https://app.opencomputer.dev) starts with **Import from GitHub** and keeps **Configure manually** available. Repository import recognizes a complete [Flue](/agent-sessions/flue) app; prompt- or skills-only folders are not converted into an agent. You can also deploy a local Flue checkout with `oc agent deploy`. Both Flue paths produce the same deployment and revision history.</Note>
