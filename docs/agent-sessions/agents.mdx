...
<Note>You can create and edit built-in runtime agents in the [dashboard](https://app.opencomputer.dev). For Flue, choose **Create agent → Import from GitHub** to build an existing repository, or deploy a local checkout with `oc agent deploy`. Both paths produce the same deployment and revision history.</Note>
...
| **+ Deploy from a repo / CLI** | keep built-in behavior in `prompt.md` + `skills/`, or keep a Flue app in a self-contained npm root | versioned [deployments and revisions](/agent-sessions/revisions); Flue repository imports also include durable build logs |
