...
After import, the dashboard opens guided setup rather than making the deployment log the next task.
Choose **Connect Slack** while the deployment runs in the background. Compact phases remain visible,
the build log expands on demand, and setup promotes **Open Slack** plus the first-message prompt when
both resources are ready. It then watches ordinary session history: before the first message it says
that it is waiting, and after managed Slack creates the session it previews the user message and
agent response with links to the full session and all sessions for that agent. Until then, the agent
page keeps **Continue setup** available.

The picker also shows existing deployment-source claims before it inspects repository content. The
uniqueness boundary is one OpenComputer agent per repository root, not one per whole repository, so a
linked `agents/support` does not block `agents/sales`. Selecting the exact linked root disables review,
names and links the owning agent, and offers an inline confirmed unlink. Unlinking stops deploy-on-push
...
truncated, select a narrower root. Candidate roots that are already linked are marked and cannot be
selected until opened or unlinked.
...
different agent while preserving the current agent and its history, then offers to review and import
