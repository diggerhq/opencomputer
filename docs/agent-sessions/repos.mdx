...
implementation itself. The dashboard uses the OpenComputer App to:

1. inspect the selected branch and root without running repository code;
2. show the exact commit, Flue entrypoint, model, Node compatibility, and non-secret variable names;
3. create an undeployed agent and its first deployment attempt; and
4. fetch that exact tracked tree in a short-lived Git box.
...
build commands with public-Internet-only egress, and emits an artifact. The Workers deployment
credential belongs to another trusted service and is never present during source or build work.

Flue repository deployment currently supports one self-contained npm root with `agent.toml`,
`package.json`, and `package-lock.json`. Automatic deployment on later pushes is not enabled;
explicitly deploy the
latest production-branch head from the agent page after changing the repository. See
