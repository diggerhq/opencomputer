...
6. Choose **Deploy agent**. A guided setup page opens immediately and keeps the deployment running in the background. It shows compact **Prepare**, **Build**, and **Deploy** progress; expand the build log only when you need it.
7. Choose **Connect Slack** as the next step. Slack authorization returns to setup, which changes to **Open Slack** as soon as both the agent and connection are ready and prompts you to send your first message.
8. Send the message in Slack. Setup detects the real managed-Slack session, previews the first exchange, and links to the full session in OpenComputer. If you leave early, use **Continue setup** from the agent page.

If the chosen repository root is already a deployment source for another agent,
the picker marks it before review and disables the review action. Open the
owning agent, choose another monorepo root, or confirm an inline unlink. The
...
# The deploy result prints the permanent Agent URL and this next command.
oc agent invoke support-triage \
  --data '{"message":"Order 2203 arrived with a torn shoulder strap. What happens next?"}'
```
...
export default defineAgent((ctx) => {
  ...
  return {
    profile: defineAgentProfile({
      instructions:
        "Triage customer requests and give the next concrete action.",
    }),
    model: DEFAULT_MODEL,
  };
});
...
export const lookupOrder = defineTool({
  ...
  run({ input }) {
    return (
      (orders as Record<string, unknown>)[input.order_id] ?? { found: false }
    );
  },
});
```
...
export default defineAgent((ctx) => {
  ...
  return {
    profile: defineAgentProfile({
      instructions:
        "Work in the sandbox only when a shell or file operation is needed.",
    }),
    model: DEFAULT_MODEL,
...
Successful framework build output stays quiet so the deployment result remains
scannable. Pass `--verbose` to stream it; if the build fails, the CLI replays
...
- Durable model-call usage is folded into turn and session usage with
  `attribution: "best_effort"`. Use it for visibility; the agent/org gateway is
  the hard spend-enforcement authority.
...
- Each production push is re-inspected at its exact commit. Valid Flue changes—including the
  entrypoint, model, variables, package, and app source—deploy within the pinned `flue-app-v1`
  profile; removing or replacing that profile stops before build and leaves the active revision
  unchanged.
