...
4. Review the detected Flue entrypoint, model, and exact commit. Give the OpenComputer agent a human-readable name; it may differ from the `agent.toml` Flue entrypoint.
5. Choose **Deploy agent**. The deployment page keeps one chronological log and summarizes progress as **Prepare**, **Build**, and **Deploy**.
