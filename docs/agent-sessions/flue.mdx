...
- **Retries and timeouts.** Flue's `durability` settings are overridden: one
  engine attempt per turn, timeout wired to the platform's turn deadline.
  Cancel, retry, and deadline behavior is the platform's; there is no second
...
- **Subagents** (`session.task()`) bundle and execute in-process, but they
  are outside the tested profile: their definitions aren't triangle-checked,
  and subagent steps may appear incompletely in the session event log.
- **Anthropic models only.** Flue's other providers (OpenAI, Google, …)
  aren't routable through the platform's credential injection yet.
...
- **Text in, text out.** Image/attachment content in Flue conversations
  isn't wired to session messages.
- **No HTTP surface.** A Flue app's own server/endpoints don't run; the
  [sessions API](/agent-sessions/overview) is how the outside world reaches
  the agent.

Each of these is a candidate to widen; the profile is versioned
