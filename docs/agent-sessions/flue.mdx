...
4. Inspect the exact commit OpenComputer detected. Give the OpenComputer agent a human-readable name; it may differ from the `agent.toml` Flue entrypoint.
5. Choose **Deploy agent**. The deployment page follows source, install, build, artifact, deploy, and verify work and keeps the log if a phase fails.
...
builder version shown during inspection and a semver dependency on the Flue CLI. npm workspaces,
files above the selected root, private registries, submodules, Git LFS, custom install/build
commands, and build-time secrets are not supported in the first repository-build profile.
...
fixed `npm ci --no-audit --no-fund` and offline `oc agent build` commands. Repository code receives
public-Internet-only egress for npm, but no GitHub token, OpenComputer key, model credential,
Workers credential, secret store, inbound port, or preview URL.

<Note>
Repository import currently creates the initial deployment from the selected production branch.
Automatic deployment of later pushes is not enabled in this first release; return to the agent and
deploy the latest branch head, or use the CLI path below, after changing the repository.
</Note>
...
fix the repository, and deploy again. The first successful deployment creates revision **#1**.
...
- Repository deployment accepts one self-contained npm root with a committed npm lockfile. Initial
  dashboard import is supported; automatic deployment on later Git pushes is not enabled yet.
...
- **Managed install or build fails**: open the deployment detail page. The safe error summary remains
  available even if log output was truncated; fix the source and deploy the production branch again.
