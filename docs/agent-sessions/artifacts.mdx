...
<Note>**Coming soon.** This page is design direction, not a shipped API. Today, large event bodies can spill to a [`content_ref`](/agent-sessions/events), and events can carry [`refs.artifacts[]`](/agent-sessions/events).</Note>
...
The runtime would get two tools:
