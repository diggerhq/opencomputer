...
<Note>**Planned.** This page describes planned API shape, not shipped behavior. Today a session works in its [sandbox](/agent-sessions/runtime-tools); for a public repo the agent clones on demand, and for a private repo OpenComputer checks it out before the first turn (see [Repos & GitHub](/agent-sessions/repos)).</Note>
