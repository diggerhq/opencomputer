...
Connect a GitHub App to let OpenComputer obtain short-lived, repo-scoped tokens for the repositories agents may work in. A **Repo** is repository identity + policy. See the [Repos guide](/agent-sessions/repos).
...
| `oc.github.installations.list(params?)` | List known installations for App-key modes; optional for setup/status. |
...
| `GET /github/installations?app_id=...` | List known installations for App-key modes; optional for setup/status. |
...
Responses **never** contain a token or secret. (SDK camelCase: `branchNamespace`, `appId`, `appSlug`, `accountLogin`.) Installations are a status/preflight surface: App-key modes can list them, while broker-mode Apps may have none because your broker owns that lifecycle. Registering a repo is **optional** under the OpenComputer App — a session can reference `"owner/repo"` directly.
