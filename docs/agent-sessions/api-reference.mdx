...
The source remains linked after import. A later GitHub push that touches the selected root on its
production branch automatically creates the next deployment. Flue ignores non-production pushes;
the current runtime has one live Worker and no preview/staged Worker.
...
with [`POST /agents/import`](#import-a-flue-repository); import deploys the reviewed production head,
