...
with [`POST /agents/import`](#import-a-flue-repository); it deploys only an explicitly requested
production-branch head and has no preview/staged Worker. `POST …/revisions` is inline-only
sugar.
