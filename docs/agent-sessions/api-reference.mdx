...
**`Turn`** — `{ id, state, yield_reason?, result_event_id?, error?, started_at?, completed_at? }`. `yield_reason` ∈ `completed · needs_input · error · budget_exceeded · deadline_exceeded · max_turns · canceled` (`needs_input` → answer by [steering](#steer)).
...
| `error_code?` `error_message?` `retryable?` | `error_code`: `source.{auth_required, repo_not_selected, permission_missing, mint_failed, ref_not_found, sha_mismatch}` |
