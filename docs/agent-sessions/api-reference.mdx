...
SourceSummary = { name, status, path, sha,        // returned today
                  resolved_sha?, error_code?, error_message?, retryable? }  // forward-looking
// sanitized; in the create response + GET …/sources
//   status today: pending | materializing | resolved | failed | auth_required
//     (unavailable is reserved/forward-looking)
...
Connect a GitHub App to let OpenComputer obtain short-lived, repo-scoped tokens for the repositories agents may work in. A **Repo** is repository identity + policy. See the [Repos & GitHub guide](/agent-sessions/repos).

The OpenComputer-owned App is the current simplest path and is installed from its GitHub App install URL, not by minting an install URL through the SDK. Bring-your-own GitHub App support adds more App records later; repo and session shapes stay the same. **No credential passes through this API.**

GitHub App `mode` values: `oc_app` (OpenComputer owns and mints), `byo_stored_key` (your
App key is encrypted in OpenComputer; OpenComputer mints), and `byo_broker` (your backend
owns the App key and returns operation-scoped short-lived tokens).
...
| Call | Purpose |
| --- | --- |
| `oc.github.apps.list()` | List GitHub Apps available to your org. _(coming soon)_ |
| `oc.github.installations.list(params?)` | List installations for `oc_app` / `byo_stored_key`. _(coming soon; the OC App is resolved automatically — not needed to use a repo)_ |
...
| Method · Path | Purpose |
| --- | --- |
| `GET /github/apps` | List GitHub Apps available to your org. _(coming soon)_ |
| `GET /github/installations?app_id=...` | List installations for `oc_app` / `byo_stored_key`. _(coming soon; OC App resolved automatically)_ |
...
Repo create body: `owner`, `repo`, `provider?` (`github`, default), `name?` (a handle), `app_id?`, `defaults?`. **No `auth`** — auth resolves through the GitHub App. **Idempotent by `(provider, owner, repo)`** (201 fresh / 200 repeat). `defaults`: `{ branch_namespace?, allow? }` — `allow` is a **GitHub permission ceiling** (e.g. `contents:read`, `pull_requests:write`).

```
Repo               = { id, provider, owner, repo, name?, app_id?, defaults?, created_at, updated_at }
GitHubApp          = { id, provider, mode, status, app_slug?, app_name?, is_default?, created_at }
GitHubInstallation = { id, app_id, account_login, status, repository_selection?, created_at }
```

Responses **never** contain a token or secret. (SDK camelCase: `branchNamespace`, `appId`, `appSlug`, `accountLogin`.) Installations are a status/preflight surface: `oc_app` and `byo_stored_key` Apps can list them, while `byo_broker` Apps may have none because your backend owns that lifecycle. Registering a repo is **optional** under the OpenComputer App — a session can reference `"owner/repo"` directly.
