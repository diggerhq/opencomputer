...
## GitHub App installations & Repos

Install the **OpenComputer GitHub App** to let OpenComputer mint short-lived, repo-scoped tokens for the GitHub accounts and repositories you select. A **Repo** is repository identity + policy. See the [Repos guide](/agent-sessions/repos).

The OC-owned App is installed from the dashboard or GitHub, not by minting an install URL through the SDK. Bring-your-own GitHub App support will add a separate GitHub App entity later; repo and session shapes stay the same. **No credential passes through this API.**
...
| Call | Purpose |
| --- | --- |
| `oc.github.installations.list()` | List OpenComputer GitHub App installations available to your org. |
...
| Method · Path | Purpose |
| --- | --- |
| `GET /github/installations` | List OpenComputer GitHub App installations available to your org. |
...
Repo create body: `owner`, `repo`, `provider?` (`github`, default), `name?` (a handle), `installation_id?`, `defaults?`. **No `auth`** — auth comes from the GitHub App installation. **Idempotent by `(provider, owner, repo)`** (201 fresh / 200 repeat). `defaults`: `{ branch_namespace?, allow? }` — `allow` is a **GitHub permission ceiling** (e.g. `contents:read`, `pull_requests:write`).

```
Repo               = { id, provider, owner, repo, name?, installation_id?, defaults?, created_at, updated_at }
GitHubInstallation = { id, account_login, status, repository_selection?, created_at }
```

Responses **never** contain a token or secret. (SDK camelCase: `branchNamespace`, `installationId`, `accountLogin`.) Registering a repo is **optional** under the OpenComputer App — a session can reference `"owner/repo"` directly.
