...
| Call | Purpose |
| --- | --- |
| `oc.github.connections.installUrl(params?)` | Create a GitHub App installation URL. |
...
| Method · Path | Purpose |
| --- | --- |
| `POST /github/connections/install-url` | Create a GitHub App installation URL. |
...
Install URL body: `account?`, `redirect_url?`; response `{ url }`. Redirect the user to `url`, then list connections after GitHub returns to your app.

Repo create body: `owner`, `repo`, `provider?` (`github`, default), `name?` (a handle), `connection_id?`, `defaults?`. **No `auth`** — auth comes from your GitHub connection. **Idempotent by `(provider, owner, repo)`** (201 fresh / 200 repeat). `defaults`: `{ branch_namespace?, allow? }` — `allow` is a **GitHub permission ceiling** (e.g. `contents:read`, `pull_requests:write`).
...
Responses **never** contain a token or secret. (SDK camelCase: `branchNamespace`, `connectionId`, `accountLogin`.) Registering a repo is **optional** under the OpenComputer App — a session can reference `"owner/repo"` directly.
