...
SessionSource =                                    // a sources[] entry (SDK camelCase)
    { repo, ref, sha, name? }                      // registered repo (repo_… or "owner/repo")
  | { url, ref, sha, name?, auth }                 // inline; auth = the per-session credential
SourceAuth =
...
SourceSummary = { name, status, path, sha }        // sanitized; echoed in the create response
```
...
The response **never** contains a token or secret. (SDK is camelCase: `brokerUrl`, `branchNamespace`.)
