...
| `session.sources` | Source checkout status — `SourceSummary[]`. |
...
SessionSource =                                    // a sources[] entry — WIRE shape (SDK is
    { repo, ref, sha, name? }                      //   camelCase: brokerUrl, expiresAt)
  | { url, ref, sha, name?, auth }                 // name defaults to the repo name; type defaults "git"
SourceAuth =
    { type: "token_broker", broker_url, grant }    // grant: a short-lived reference, not a token
  | { type: "risky_short_lived_token", token, expires_at }   // discouraged; short-lived only
...
A reusable, owner-scoped connection to a Git repository — referenced from a session's `sources`. See the [Repos guide](/agent-sessions/repos) for the security model.
...
| Call | Purpose |
| --- | --- |
| `oc.repos.create(params)` | Register (get-or-create) a repo connection. |
| `oc.repos.get(idOrSlug)` · `oc.repos.list()` | Fetch / list. |
| `oc.repos.update(id, params)` | Update `auth` / policy `defaults`. |
...
| Method · Path | Purpose |
| --- | --- |
| `POST /repos` | Register (get-or-create) a repo connection. |
| `GET /repos/:id` · `GET /repos` | Fetch / list. |
| `PATCH /repos/:id` | Update `auth` / policy `defaults`. |

</Tab>

</Tabs>

Create body: `owner`, `repo`, `provider?` (`github`, default), `name?` (a handle), `auth`, `defaults?`. **Idempotent by `(provider, owner, repo)`** — a fresh register returns **201**, a repeat returns the existing repo with **200**. Stores **no GitHub token** — only the broker URL, repo identity, and policy.

`auth` — how OpenComputer obtains tokens (no standing credential is stored):
- `{ "type": "token_broker", "broker_url": "https://…" }` — your [broker](/agent-sessions/repos#what-you-provide) endpoint; OpenComputer signs each request to it, verified against its published JWKS.

`defaults` (optional policy): `{ "branch_namespace"?: "oc/{session}/", "allow"?: ["contents:read", "pull_requests:write", …] }` — `allow` is a ceiling on what any request may mint.

```
Repo = { id, provider, owner, repo, name?, broker_url?, defaults?, created_at, updated_at }
```

The response **never** contains a token or secret. (SDK is camelCase: `brokerUrl`, `branchNamespace`.) `defaults.allow` is a **GitHub permission ceiling** (e.g. `contents:read`, `pull_requests:write`) — not a free-form list.

**Broker request** (the contract your `broker_url` implements). At materialization OpenComputer `POST`s your broker a **signed** request (verify against OpenComputer's published JWKS):

```
→  { owner_id, repo, operation, permissions, ref, sha }   // signed by OpenComputer
←  { token, expires_at, permissions }                     // a JIT installation token
```

**Bind `owner_id` to your OpenComputer org** (allowlist or a one-time registration) so a foreign org can't point at your broker URL and mint tokens for your installation. Mint only the intersection of your grant/policy and the requested scope — never broader. See the [Repos guide](/agent-sessions/repos#what-you-provide).
