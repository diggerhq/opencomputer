...
Connect a GitHub App to let OpenComputer obtain short-lived, repo-scoped tokens for the repositories agents may work in. A **Repo** is repository identity. See the [Repos & GitHub guide](/agent-sessions/repos).
...
| `oc.repos.update(id, params)` | Update `name` / `appId`. |
...
| `PATCH /repos/:id` | Update `name` / `app_id`. |

</Tab>

</Tabs>

Repo create body: `owner`, `repo`, `provider?` (`github`, default), `name?` (a handle), `app_id?`. **No `auth`** — auth resolves through the GitHub App. **Idempotent by `(provider, owner, repo)`** (201 fresh / 200 repeat). `app_id` is optional — the GitHub App this repo resolves to; defaults to your org's default App, then the OpenComputer App.

```
Repo               = { id, provider, owner, repo, name?, app_id?, created_at, updated_at }
```

Responses **never** contain a token or secret. (SDK camelCase: `appId`.) Registering a repo is **optional** under the OpenComputer App — a session can reference `"owner/repo"` directly.
