...
Create body: `name`, `prompt`, `model`, `runtime?` (`claude` | `codex`), `key?`, `credential?`, `limits?`. Patch accepts `prompt?`, `model?`, `key?`, `credential?`, and `limits?` — **not `runtime`, which is fixed at creation**. The `runtime` / `model` provider must agree (`claude`↔`anthropic/…`, `codex`↔`openai/…`), validated at create. `credential` is a closed value: a `cred_…` id (BYO, must match the runtime's provider) **or** the literal `"managed"` — run the model **via OpenComputer, billed to your credits**, which bypasses the credential provider match (the `model` still must match the runtime). `key` and `credential` are mutually exclusive.
...
Pass `key` **or** `credential` (`"managed"` or a `cred_…` id), or neither → org default (Managed when your org has no default key). The agent's `limits` are the defaults for its sessions; a session's own `limits` override. Each agent has a monotonic `revision` (bumped on PATCH) that sessions pin in `agent_snapshot.revision`, so you can tell which version produced a run.
...
Credential body: `provider` (`anthropic` | `openai`), `key`, `name?`, `is_default?`. Keys are write-only. A BYO credential is optional — agents can run **Managed** (`credential: "managed"`, billed to your credits, no key), the default for new orgs. See the [Credentials](/agent-sessions/credentials) guide for Managed vs BYO, resolution, rotation, and the dashboard.
