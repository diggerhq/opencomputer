...
SourceSummary = { name, status, path, sha, resolved_sha?,
                  error_code?, error_message?, retryable? }
// sanitized; in the create response + GET …/sources
//   status: pending | materializing | resolved | failed | unavailable | auth_required
//   error_code examples: source.auth_required, source.auth_ambiguous,
//   source.permission_missing, source.sha_mismatch, source.timeout
```
...
| `oc.github.installations.list(params?)` | List known installations for `oc_app` / `byo_stored_key`; optional for setup/status. |
...
| `GET /github/installations?app_id=...` | List known installations for `oc_app` / `byo_stored_key`; optional for setup/status. |
