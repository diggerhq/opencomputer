...
| `POST /agents/import` | Atomically create a Flue agent from an inspected GitHub source and enqueue its first deployment. |
...
`POST /agents/import` is the Flue repository-import command described below. It deliberately creates
...
Repository inspection and import are currently REST/dashboard surfaces. Inspection resolves the
moving production branch to one commit and validates bounded metadata without executing repository
code:
...
The response includes `repository`, normalized `root`, `production_ref`, exact `sha`, the manifest
projection (`entrypoint`, `model`, `runtime`, non-secret `vars`), package and lockfile metadata,
published builder Node, source counts, `variable_names`, and safe `warnings`. It never includes a
repository token or source bytes.

After review, submit the same source coordinates with a human-readable agent name. The name is
independent of the Flue entrypoint in `agent.toml`.
...
The server repeats authoritative inspection, then creates `{ agent, source, deployment }` atomically.
The deployment starts at `accepted`; the agent is visible but cannot start sessions until it reaches
`ready`. Reusing the same idempotency key and command returns the same import; reusing it with
different fields returns `409 idempotency_conflict`. A name or repository-root conflict also returns
`409` and identifies the existing agent when available.
...
with [`POST /agents/import`](#import-a-flue-repository); its first profile deploys only an explicitly
requested production-branch head and has no preview/staged Worker. `POST …/revisions` is inline-only
sugar.
