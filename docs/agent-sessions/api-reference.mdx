...
Credential body: `provider`, `key`, `name?`, `is_default?`. Keys are write-only. A credential is **required**; sessions run on your provider key. See the [Credentials](/agent-sessions/credentials) guide for resolution, rotation, and the dashboard.
