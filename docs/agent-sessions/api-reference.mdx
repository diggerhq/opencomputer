...
| `error_code?` `error_message?` `retryable?` | on failure. Codes: `auth_required` (App not installed), `repo_not_selected` (install doesn't cover the repo), `permission_missing`, `mint_failed`, `ref_not_found`, `sha_mismatch` |
...
|---|---|
| `id` `created_at` | |
| `state` | progresses `accepted → queued → fetching → validating → uploading → ready`; terminal: `failed`, `superseded` (a newer deploy won), `skipped` (content unchanged) |
