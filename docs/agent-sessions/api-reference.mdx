...
```
Session = { id, status, head, input_cursor, agent_id, credential_id,
            key?, metadata?, created_at,
            agent_snapshot: { prompt_hash, model, runtime, revision },   // revision = pinned #
...
Source = { repo, ref, sha, name? } | { url, ref, sha, name?, auth }
  auth (inline only) = { type: "risky_short_lived_token", token, expires_at }   // can't refresh
Summary= { name, status, path, sha, resolved_sha?, error_code?, error_message?, retryable? }
  status     = pending | materializing | resolved | failed | auth_required
  error_code = source.* — auth_required, repo_not_selected, permission_missing,
                          mint_failed, ref_not_found, sha_mismatch
```

`yield_reason` = `completed | needs_input | error | budget_exceeded | deadline_exceeded | max_turns | canceled` (why the turn ended; `needs_input` → answer by [steering](#steer)).
...
```
Event = { id, seq, ts, session, actor, type, level, body, turn_id?,
...
`input`: `{ type:"inline", prompt, model?, skills?, runtime? }` (the **complete** behavior — omitted `skills` = none) **or** `{ type:"github", ref?, sha? }` (deploys the [linked repo](#deploy-from-a-repo); `repo`/`path` from the link). `activate:false` **stages** it. Inline = sync (`ready` + `revision_id`); GitHub = async (`queued`). `POST …/revisions` is inline-only sugar.

```
Deployment = { id, state, revision_id|null, active, source?, error?, created_at }
//   state:  accepted | queued | fetching | validating | uploading
//           | ready | failed | superseded | skipped
...
```
Revision = { id, number, digest, prompt, model, skill_bundle_digest?, runtime_ref,
             active, created_at, files?: [{ path, mode, size }] }   // files on GET …/:rev
```
...
```
DeploymentSource = { agent_id, repo_id, path, production_ref, status,
                     latest_seen_sha?, active_deployed_sha?, … }
