...
Three ways — each produces a new revision:
...
In the [dashboard](https://app.opencomputer.dev), the agent's **Skills** panel uploads the same `.zip`. This keeps the agent's current prompt + model and replaces its skills.
</Tab>
...
Put a `skills/` directory in your agent directory and [deploy from GitHub](/agent-sessions/deploy-from-github) (push to deploy) or `oc agent deploy` from a local checkout:
