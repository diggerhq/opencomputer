...
A key is a hard secret the moment it reaches us, and stays one:

- **Encrypted, write-only** — held in a dedicated [secret store](/sandboxes/secrets) (AES-256-GCM), isolated from the app database, and never returned by the API (reads expose only a `last4` hint).
- **Never in the sandbox** — the runtime sees an opaque placeholder; a host-side proxy swaps in the real key only on the outbound call to the provider, scoped to that host (`api.anthropic.com` / `api.openai.com`) by an egress allowlist.
- **Rotatable in place** — replace the value any time and it reaches **running** sessions, so a compromised key retires without restarting work ([rotation](#rotation)).
