...
test("init creates a multi-agent-ready hello-world agent by default", async () => {
  ...
  try {
    ...
    assert.match(
      ...
      /First run, in order:[\s\S]*npx opencomputer login[\s\S]*npx opencomputer link --create-project[\s\S]*npm run deploy -- --watch[\s\S]*opencomputer\/\.env\.example[\s\S]*--value-stdin/,
    );
    assert.match(
